...
contract Harberger {
  function payDefault(
    ...
    (nextPrice, taxes) = Harberger.getNextPriceDefault(perwei, period, prevPrice);
    require(taxes <= prevPrice, "taxes too high");
  }

  function getNextPriceDefault(
    Perwei memory perwei,
    Period memory period,
    uint256 price
  ) internal pure returns (uint256, uint256) {
    uint256 taxes = taxPerBlockDefault(perwei, period, price);
